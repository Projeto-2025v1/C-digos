...
n = 20
dim_sistema = 6 + n + 1  # Dimensão total do sistema
...
def sistema_edo(t, y):
    ...
    dydt[5] = -alphaHP * x6 * (x5 - vHP1) * (x5 - vHP2) - \
              ...
              kAVHP * x5 + kAVHPtau * (x3 - tauAVHP * x4)
...
def jacobiano(t, y):
    ...
    # dx4/dx1_atrasado (último elemento dos estados de atraso)
    J[3, 6+n] = kSAAVtau
    ...
    # dx6/dx3 e dx6/dx4
    J[5, 2] = kAVHPtau
    J[5, 3] = -kAVHPtau * tauAVHP

    # Derivadas para o sistema de atraso
    for i in range(1, n):
    ...
    # Conexão entre x2 e o primeiro elemento do atraso
    J[6, 1] = 1
...
if __name__ == "__main__":
    ...
    print(f"Dimensão do sistema: {dim_sistema}")
    ...
    plt.xlabel('Número de Iterações')
    plt.ylabel('Expoente de Lyapunov')
    plt.title('Evolução de Todos os Expoentes de Lyapunov')
    plt.grid(True, alpha=0.3)
